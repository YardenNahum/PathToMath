...
const subjectMap = {
    ...
    "Percentage": {
        "mathAction": "%",
        "function": (a, b) => Math.floor(a / b * 100)
    }
};
...
const generateQuestions = (gameSubject, gameLevel, numOfQuestions = 1, numOfOptions = 4) => {
    ...
    const generateVariable = (gameLevel) => {
        ...
        switch (gameSubject) {
            ...
            case "Percentage":
                 ...
                 *  5th grade: null, 6th grade: 1-100 
                 */
                if (gameLevel === 6) {
                    maxValue = 100;
                    minValue = 1;
    ...
    const generateOption = (answer) => {
        ...
        const optionValue = Math.round(Math.random() * (maxBorder - minBorder) + minBorder);
    ...
    const makeQuestion = () => {
        ...
        switch (gameSubject) {
            ...
            case "Division":
                ...
                break;
            case "Percentage":
                // Generate a base number (var2) that's a multiple of 10 or 25 for easier calculations
                let baseNumbers = [10, 20, 25, 50, 100, 200, 500, 1000];
                let baseNumber = baseNumbers[Math.floor(Math.random() * baseNumbers.length)];
                var2 = {
                    value: baseNumber,
                    textValue: numberToString(baseNumber)
                };

                // Generate a percentage (var1) that will result in a clean number
                var1 = generateVariable(gameLevel);
                // Ensure var1 is a multiple of the base number's divisor
                var1.value = Math.floor(var1.value / (100 / baseNumber)) * (100 / baseNumber);
                var1.textValue = numberToString(var1.value);
        ...
        if (gameSubject === "Percentage") {
            questionText = `What percent is ${var1.textValue} of ${var2.textValue}?`;
        } else {
