  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5 w-full max-w-screen-lg p-1">
      ...
      {levels.map(({ levelNum, color, canPlay }) => (
        <button
          key={`level-${levelNum}`}
          className={`${color} ${canPlay ? 'cursor-pointer' : ''} text-white text-2xl font-bold py-4 px-4 rounded-4xl shadow-md flex items-center justify-center transition-transform duration-200 ${canPlay ? 'hover:scale-105 hover:animate-pulse' : ''
            }`}
          disabled={!canPlay}
